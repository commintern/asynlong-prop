...
Rcpp::List Xbar_c(const arma::rowvec& gamma,
  ...
  arma::mat XXtZbar_temp = mat(p+1,p+1);
...
// [[Rcpp::export]]
Rcpp::List P_D_c(const arma::rowvec& gamma,
                const arma::rowvec& theta,
                Rcpp::ListOf<NumericMatrix>& kerMat,
                Rcpp::ListOf<NumericVector>& meas_times,
                Rcpp::ListOf<NumericMatrix>& covariates,
  ...
  arma::vec temp_meas_time_i, temp_meas_time_l, temp_countprocess_l,temp_dlambda, temp_mu0;
  arma::vec censorind, temp_S0;
  double weight;
  arma::uvec temp_uvec;
        ...
        for (l = 0; l < n; l++) {
            ...
            for (k = 0; k < Kn; k++) {
              if (abs(temp_kermat(j, k)) > 1e-10) {
                ...
                weight = as_scalar(temp_kermat(j, k) * exp(gamma * temp_cov.col(k))) * temp_dlambda(j);

                temp_X_diff = X_temp - trans(Xbar_temp.row(j));

                P_temp =  P_temp + temp_X_diff * trans(temp_X_diff.head(p)) *
                  (temp_mu0(j)+as_scalar(theta * X_temp)) * weight;

                D_temp =  D_temp + temp_kermat(j, k) * temp_X_diff.head(p) * trans(temp_X_diff.head(p));
  ...
  return Rcpp::List::create(Rcpp::Named("P") = P_temp/n,
                            Rcpp::Named("D") = D_temp/n);
}
...
Rcpp::List long_asy_c(const arma::rowvec& gamma,
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < Jn; j++) {
      ...
      for (k = 0; k < Kn; k++) {
        ...
        if (abs(temp_kermat(j, k)) > 1e-10) {
          ...
          //temp_part1 =  temp_part1 + X_temp  * weight;

          //cout << "temp_part1：";

          //cout << temp_part1.t() << endl;
    ...
    //res_temp_ind.print();
    Sigmat = Sigmat + temp_part1 * temp_part1.t();
  }
  Sigmat = Sigmat/n/n;
  Bmat = Bmat/n;
