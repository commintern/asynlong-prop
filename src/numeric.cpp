...
//   double TOLF=1.0e-16, TOLX=EPS, STPMAX=100.0, TOLMIN=1.0e-16;
//   int MAXITS = 1000;
//   check = false;
...
struct nl_fun {
  arma::vec gamma;
  nl_fun(arma::vec gg) : gamma(gg) {}
  arma::vec operator()(arma::vec x) {
    return arma::ones(x.size()) - gamma % exp(-exp(x % gamma));
...
arma::vec optimtest(const arma::vec& z_in, const arma::vec& gamma_in ){
  ...
  arma::vec z = z_in;
  arma::vec gamma = gamma_in;

  nl_fun test_fun = nl_fun(gamma_in);
  bool check = false;
  //Rcpp::Rcout << test_fun(z) << std::endl;

  arma::vec res = broyden<nl_fun>(z, test_fun, check);
